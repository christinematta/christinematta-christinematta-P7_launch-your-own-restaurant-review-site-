...
class PlaceDetails extends Component {
    ...
    onClickReview(e) {
        this.setState({showReview: !this.state.showReview}); // toggle
    }
                ...
                <div style={{fontWeight: "bold"}}>
                    <form onSubmit={this.onClickSaveReview}>
                        ...
                        <input value={this.state.newPlace.name}
                               ...
                               style={{width: "100%", height: "40px", fontSize: "medium"}} required/>
                        <br/><br/>
                        ...
                        <textarea required value={this.state.newPlace.text} placeholder="Enter your comment"
                                  style={{width: "100%", height: "60px", fontSize: "medium"}}
                                  onChange={(e) => {
                                      const value = e.target.value;
    ...
    renderReviewDetails(review) {
        return (<div>
            <img src={review.profile_photo_url === null ? profilePhoto : review.profile_photo_url}
                 style={{width: "70px", height: "70px"}} alt="RestaurantImage"/>
            <h5 style={{
                marginLeft: "19%",
                marginTop: "-11%",
                marginBottom: "10%",
                fontsWeight: "large"
            }}> {review.author_name}</h5>
            ...
            <Grid container>
                            ...
                            <Carousel dynamicHeight showThumbs={false}>
                                {place !== null && place.photos !== undefined ?
                                    place.photos.map(item =>
                                        <div>
                                            <img src={item} alt="" style={{width: "800px", height: "400px"}}/>
                                        </div>) : null}
                            </Carousel>
                        </div>
                ...
                <Grid item xs={4}>
                    <div className="rest-layout">
                        ...
                        <input type="button" className="review-button"
                               value={this.state.showReview ? "Cancel" : "Add Review"} onClick={this.onClickReview}/>

                        {this.renderReviewForm()}

                        {place !== null && place.reviews !== undefined ?
                            place.reviews.map((item, index) => {
                                return index < 5 ? this.renderReviewDetails(item) : null
                            }) : null}
                    </div>
                </Grid>
